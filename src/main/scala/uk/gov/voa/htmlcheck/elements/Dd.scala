...
case class Dd(protected val element: Element)
  extends HtmlElement
...
object Dd {
  ...
  implicit def elementWrapper(element: Element): HtmlCheckError Xor Dd =
    if (element.tagName() != "dd")
      Left(ElementOfWrongType("dd", element.tagName(), IdAttribute(element)))
    else
      Right(Dd(element))
}
