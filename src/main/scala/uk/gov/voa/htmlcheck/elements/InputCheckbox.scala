...
import uk.gov.voa.htmlcheck.elements.InputCheckbox.CheckedAttribute
import uk.gov.voa.htmlcheck.{ElementOfWrongType, HtmlCheckError}

import scala.language.implicitConversions

case class InputCheckbox(protected val element: Element) extends Input {
...
object InputCheckbox {
  ...
  implicit def elementWrapper(element: Element): HtmlCheckError Xor InputCheckbox =
    if (element.tagName() != "input" || !TypeAttribute(element).exists(_.value == "checkbox"))
      Left(ElementOfWrongType("input-checkbox", s"${element.tagName()}${TypeAttribute(element).map(t => s"-$t").getOrElse("")}", IdAttribute(element)))
    else
      Right(InputCheckbox(element))
